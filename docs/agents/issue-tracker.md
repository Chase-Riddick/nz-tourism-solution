...
Chosen deliberately (2026-08-18): the repo is public, but it is a hypothetical
marketing site built for one person to react to. It is not soliciting contributions,
so an external-PR queue would only ever sit empty. Flip to `yes` if that changes.
